...
contract Incubator is Ownable {
    ...
    event IncubatorStore(address indexed from, uint256 incubatorId, uint256 tokenId);
    event IncubatorBreed(address indexed from, uint256 incubatorId, uint256 first, uint256 second);
    ...
    function breed(uint256 _incubatorId, uint256 _tokenId) public {
        ...
        IERC1155(nft).safeTransferFrom(_msgSender(), blindBox, _tokenId, 1, abi.encode(uint256(1)));
