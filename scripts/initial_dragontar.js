...
async function main () {
  ...
  const result1 = await nftFactoryInstance.methods.registerAttribute(2, genericAttrAddress).send({ from: caller });
  ...
  const result2 = await genericAttrInstance.methods.grantRole(ATTACH_ROLE, nftFactoryAddress).send({ from: caller });
